...
# Link text-generation-webui's model folder to $VOLUME so downloaded models persist
"$SCRIPTDIR"/models-on-workspace.sh
...
do
  ...
  if [[ ! -f $VOLUME/do.not.launch.UI ]]
  then
    # Launch the UI in a loop forever, allowing UI restart
    if [[ -f /tmp/text-gen-model ]]
    then
